...
public class LC2022W1_检查二维数组存在某个值 {
    static class Solution {
        public boolean checkValid(int[][] matrix) {
            int n = matrix.length;
            for (int[] ints : matrix) {
                if (!check(n, ints)) return false;
            }
            if (!shuCheck(n, matrix)) return false;

            return true;
        }


        public boolean check(int n, int[] array) {
            int[] check = new int[n];
            Arrays.fill(check, -1);
            int flag = 0;
            for (int i : array) {
        ...
        public boolean shuCheck(int n, int[][] array) {
            int[] check = new int[n];
            for (int i = 0; i < n; i++) {
                Arrays.fill(check, -1);
                int flag = 0;
                for (int j = 0; j < n; j++) {
                    if (check[array[i][j] - 1] == -1) {
                        check[array[i][j] - 1] = 1;
                        flag++;
                    }
                }
